...
int Ambulance::amb_id = 0;
    ...
    int main(){
        ...
        std::vector<Ambulance*> solution = {&ambulance_1, &ambulance_2};

//    NeighbourSelect(ambulance_1, ambulance_2, solution);
        //swap1(ambulance_1, ambulance_2, 0, 0);

        std::map<Ambulance, int> pair1 = {{ambulance_1, 0}};
        std::map<Ambulance, int> pair2 = {{ambulance_2, 0}};

        std::vector<std::map<Ambulance, int>> mapVector = {pair1, pair2};
